	...
	while(1)
	{
		...
		if (strcmp(ptr, "List\n") == 0)
		{
			...
			else
			{
				...
				printf("client %s's balance: %d\n", name[client_idx], user_bal[client_idx]);
				sprintf(message, "%d\n", user_bal[client_idx]);
				ptr = strchr(ptr, '\0');
        ...
        ptr = strtok(temp, "#");
			...
			else
			{
				...
				if(ptr != NULL)
				{
					user_bal[client_idx] += atoi(amount);
					for(int i = 0; i < user_cnt; i++)
					{
						if(strcmp(sender, name[i]) == 0)
						{
							payer = i;
							user_bal[i] -= atoi(amount);
							flag = 1;
							break;
						}
					}
				...
				if(flag == 1)
				{
					...
					printf("payer: %s\n", name[payer]);
					printf("payee: %s\n", name[client_idx]);
					strcpy(message, "server confirm payment!!");
				}
				else
				{
